...
cli: gox cli-linux cli-darwin cli-windows
	...
	mv $(linux_out_dir)/staging.zip $(compiled_dir)/metrics_relay_$(version)_linux_amd64.zip
	(cd $(darwin_out_dir) && zip -r staging.zip $(bin_name))
	mv $(darwin_out_dir)/staging.zip $(compiled_dir)/metrics_relay_$(version)_darwin_amd64.zip
	(cd $(windows_out_dir) && zip -r staging.zip $(bin_name).exe)
	mv $(windows_out_dir)/staging.zip $(compiled_dir)/metrics_relay_$(version)_windows_amd64.zip
